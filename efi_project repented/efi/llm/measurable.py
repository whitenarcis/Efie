...
(в будущем — efi/dashboard/) подписывается на события через sink; ни
провайдер, ни роутер ничего не знают про дашборд или способ отображения.

Не подключена автоматически в efi/llm/router.py/efi/app.py — LLMRouter
создаёт провайдеров лениво сам (`_provider_for`), и для прозрачного
оборачивания потребовалось бы либо передавать router фабрику провайдеров,
либо оборачивать после создания. Модуль полностью рабочий и покрыт тем же
контрактом LLMProvider — подключить можно, обернув конкретный провайдер
перед тем как передать его туда, где он используется напрямую (в обход
роутера), либо расширив LLMRouter параметром `metrics_sink` следующим шагом.
"""
