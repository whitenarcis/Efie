...
class DevTaskKind(StrEnum):
    """
    Какого рода работа. Очередь одна, работы две — и путать их нельзя.

    PROJECT — свой проект с нуля: замысел, файлы, публикация
              (efi/dev/engine.py). Кода ещё нет, всё принадлежит ей.
    SWE     — работа с уже существующим кодом, чаще чужим: разобраться,
              поправить точечно, проверить запуском, оставить ветку
              (efi/dev/swe_engine.py). Здесь нельзя переписывать и нельзя
              ошибаться молча.
    """
...
class DevTask(BaseModel):
    ...
    idea: str
    #: Свой проект с нуля или работа с существующим кодом. От этого зависит,
    #: какой конвейер возьмёт задачу, — см. DevTaskKind.
    kind: DevTaskKind = DevTaskKind.PROJECT
    #: Откуда брать код для работы: ссылка на репозиторий или локальный путь.
    #: Только для SWE-задач; у собственного проекта источника нет — он ещё
    #: не существует.
    source: str = ""
    #: Ветка, в которую легла работа. Появляется, когда проверки позеленели.
    branch: str = ""
    ...
    #: возвращается».
    revisions: int = 0
    #: Сколько раз конвейер уже брался за задачу. Ограничивает повторы после
    #: временных отказов (429, оборванная сеть) — см. efi/dev/worker.py.
    attempts: int = 0
    #: Файлы, уже написанные и прошедшие проверку в прошлых заходах: путь ->
    #: содержимое. Повторный заход не переписывает их заново — работа,
    #: пережившая заход, переживает и его провал.
    artifacts: dict[str, str] = Field(default_factory=dict)
    #: Сколько раз она возвращалась к этому проекту после того, как бросила.
    #: Ограничивает второе дыхание — см. efi/dev/worker.py.
    revivals: int = 0
