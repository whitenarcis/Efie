...
def _build_proactive_brevity_block(notification: Notification) -> str:
    ...
    return (
        ...
        "про него.\n"
        "ЗАПРЕЩЕНО: «эй», «ты там живой?», «как дела?», «не утонул в коде?», «не сдох?», «ты там "
        "не умер?» и любые другие способы спросить «ты ещё здесь». Это не разговор, а проверка "
        "связи, и читается она именно так — особенно вторым сообщением подряд.\n"
